...
include_once('inc/connection.inc.php');
    ...
    $query = "select*from tblCompetitions order by competitionTitle";
...
if (isset($_POST['submit']))
{
    ...
    foreach ($_POST['athleteID'] as $athleteID);

    //  SQL Statement to delete record
    $query = "DELETE FROM tblCompetitions WHERE competitionID = '$competitionID'";

    //Perform SQL Query
    $result = $db->query($query);
    if ($result)
      header( "entrycompetitionchoice.php" );
        $db->close();
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <section id="form" class="bg-light" style="padding-top:1rem">
        <form action="#" method="post" class="entryform">
            <ul>
                  <li>
                <Label for="competitionID[]">Article Title</label>
                <select name="competitionID[]">
                  ...
                  <?php
                        ...
                        echo '<option value="' . $row['competitionID'] . '">' . $row['competitionTitle'] . '</option>' . "\r\n";
                    ?>
                    <?php
                        }
                    ?>
                </select>
                <span>Choose what Athlete to delete</span>
            </li>
            <li>
