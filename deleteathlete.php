...
if (isset($_POST['submit']))
{
    ...
    $query = "DELETE FROM tblAthletes WHERE athleteID = '$athleteID'";

    //Perform SQL Query
    $result = $db->query($query);
    if ($result)
      header( "entryathletechoice.php" );
        $db->close();
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <section id="form" class="bg-light" style="padding-top:1rem">
        <form action="#" method="post" class="entryform">
            <ul>
                  <li>
                <Label for="athleteID[]">Athlete Name</label>
                <select name="athleteID[]">
                  ...
                  <?php
                        ...
                        echo '<option value="' . $row['athleteID'] . '">' . $row['athleteFirstName'] . ' ' . $row['athleteSurname'] . '</option>' . "\r\n";
                    ?>
                    <?php
                        }
                    ?>
